...
let indiaTexture = new THREE.TextureLoader().load('./IMAGES/india.png');
let indiaGeo = new THREE.SphereGeometry(10, 32, 32);

let indiaMaterial = new THREE.MeshBasicMaterial({
    // color: 0xff0000,
    map: indiaTexture,
    transparent: true,
    opacity: 0,
...
let india = new THREE.Mesh(indiaGeo, indiaMaterial);

india.scale.set(1, 1, 1);
earth.add(india)
...
function onWindowClick(event) {
    ...
    console.log(intersects[0].object.state_name);

    if (!intersects[0].object.state_name) {

        // mesh_s.material.opacity = 0;
...
function addCountryCoord(earth, country_name, country_latitude, country_longitude, continent, pk, color) {
    ...
    mesh_c.rotation.set(0.0, -lon, lat - Math.PI * 0.5);

    mesh_c.userData.country_name = country_name;
    mesh_c.userData.color = color;
    mesh_c.userData.continent = continent;
    mesh_c.userData.pk = pk;
    rotationDecider = 0
    earthcountry.add(mesh_c)

};
function addStateCoord(earth, state_name, state_latitude, state_longitude) {
    let pointOfInterest = new THREE.SphereGeometry(.1, 32, 32);
    let lat = state_latitude * (Math.PI / 180);
    let lon = -state_longitude * (Math.PI / 180);
    const radius = 10;
    const phi = (90 - lat) * (Math.PI / 180);
    const theta = (lon + 180) * (Math.PI / 180);
    ...
    mesh_s.rotation.set(0.0, -lon, lat - Math.PI * 0.5);

    mesh_s.state_name = state_name;
    // mesh.userData.color = color;
    // mesh.userData.continent = continent;
    // mesh.userData.pk = pk;
    rotationDecider = 0
    earthcountry.add(mesh_s)
